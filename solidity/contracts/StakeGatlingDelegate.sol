...
contract StakeGatlingDelegate is GatlingStorage, Ownable, IStakeGatling, MasterCaller {
    ...
    function updateRate() private {
        if(now.sub(profitRateUpdateTime).mul(updatesPerDay) >= (1 days)) {
            ...
            uint256 pendingEarn = IProfitStrategy(profitStrategy).earnTokenBalance(profitStrategy);
    ...
    function sellEarn2TokenTwice() private{
        ...
        // transfer to maker
        if(sohConfig != address(0)) {
            uint256 makerShare = _minedAmount.mul(ISohConfig(sohConfig).gatlingFeeRate()).div(1000);
            if(makerShare > 0) {
                IERC20(earnToken).transfer(ISohConfig(sohConfig).sohMaker(), makerShare);
                // reset _minedAmount
                _minedAmount = _minedAmount - makerShare;
            }
        }
        //exec swap
        ...
        emit ExecUpdateProfitEvent(tx.origin, _minedAmount);
    }
